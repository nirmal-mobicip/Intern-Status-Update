...
int addOrUpdateUser(Client_FD *cfd)
{
    ...
    if (found)
    {
        ...
        strcpy(cfds[i].key,cfd->key);
        strcpy(cfds[i].iv,cfd->iv);
...
void *thread_routene(void *arg)
{
    ...
    recv(cfd->fd,cfd->key,AES_KEY_SIZE,0);
    recv(cfd->fd,cfd->iv,AES_IV_SIZE,0);
    ...
    while (flag)
    {
        ...
        //     break;
        // }
        char cbuffer[CLIENT_BUFFER_SIZE+AES_IV_SIZE], sbuffer[SERVER_BUFFER_SIZE];
        int n;
        if ((n = recvall(cfd->fd, cbuffer, CLIENT_BUFFER_SIZE+AES_IV_SIZE, 0)) > 0)
        {
            // printf("%s\n",cbuffer);                              // for reference
            char plaintext[CLIENT_BUFFER_SIZE];
            aes_decrypt(cbuffer,CLIENT_BUFFER_SIZE+AES_IV_SIZE,cfd->key,cfd->iv,plaintext);
            // printf("%s\n",plaintext);                            // for reference
            Client_Data cdata;
            Server_Data sdata;
            unpack_client_data(&cdata, plaintext);
            ...
            char cipherbuffer[SERVER_BUFFER_SIZE+AES_IV_SIZE];

            aes_encrypt(sbuffer,SERVER_BUFFER_SIZE,cfd->key,cfd->iv,cipherbuffer);

            if (sendall(cfd->fd, cipherbuffer, SERVER_BUFFER_SIZE+AES_IV_SIZE, 0) == -1)
            {
                perror("send()");
            }
            else
