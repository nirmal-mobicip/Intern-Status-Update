...
void sort_on_frequency()
{
    for (int i = 0; i < ptr; i++)
    {
        int swapped = 0;
        for (int j = 0; j < ptr - 1 - i; j++)
        {
            if (tree_sorted[j]->count < tree_sorted[j + 1]->count || (tree_sorted[j]->count == tree_sorted[j + 1]->count && strcmp(tree_sorted[j]->word, tree_sorted[j + 1]->word) > 0))
            {
                Node *temp = tree_sorted[j];
                tree_sorted[j] = tree_sorted[j + 1];
                tree_sorted[j + 1] = temp;
                swapped = 1;
            }
        }
        if (!swapped)
